...
pub async fn memory_backfill(
    ...
    let agent_ids: Vec<openfang_types::agent::AgentId> =
        if let Some(id_str) = body.get("agent_id").and_then(|v| v.as_str()) {
                    ...
                    if let Some(clf) = state.kernel.classifier.as_deref() {
                        match clf.classify(&frag.content).await {
                            Ok(result) => {
                                ...
                                new_meta
                                    .insert("scope".to_string(), serde_json::json!(result.scope));
                            }
                            Err(e) => {
...
pub async fn health_detail(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models_cached = {
        ...
        models_dir.exists() && {
            let embedding_dir =
                models_dir.join(embedding_model.replace('/', std::path::MAIN_SEPARATOR_STR));
...
pub async fn patch_agent(
    ...
    // Persist updated entry and mirror manifest edits to disk.
    if let Some(entry) = state.kernel.registry.get(agent_id) {
        let _ = state.kernel.memory.save_agent(&entry).await;
