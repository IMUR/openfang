...
impl OpenFangKernel {
    ...
    #[cfg(feature = "memory-candle")]
    pub async fn activate_ner_driver(&mut self, model_id: &str, cuda_device: Option<u32>) {
        ...
        match CandleNerDriver::load(model_id, &self.config.home_dir, cuda_device).await {
            Ok(driver) => {
                ...
                info!(
                    model_id,
                    "NER driver activated for knowledge graph population"
                );
            }
            Err(e) => {
    ...
    pub async fn populate_knowledge_graph(
        ...
        #[cfg(feature = "memory-candle")]
        {
            let Some(ref ner) = self.ner_driver else {
                return;
            };
            ...
            match ner.extract_entities(text, 0.70).await {
                Ok(entities) => {
                    ...
                    if !entity_ids.is_empty() && !memory_id.is_empty() {
                        if let Err(err) = self
                            .memory
                            .set_metadata_entities(memory_id, entity_ids.clone())
                            .await
                        {
        ...
        for agent_id in agent_ids {
            ...
            match self.default_driver.complete(req).await {
                Ok(resp) => {
                    ...
                    let mem_ids: Vec<String> = batch.iter().map(|m| m.memory_id.clone()).collect();
    ...
    async fn try_generate_session_summary(
        ...
        match self.default_driver.complete(req).await {
            Ok(resp) => {
                ...
                meta.insert("turn_at_summary".to_string(), serde_json::json!(user_turns));
    ...
    async fn execute_llm_agent(
        ...
        #[cfg(feature = "memory-candle")]
        {
            ...
            self.populate_knowledge_graph(agent_id, &result.response, memory_id_for_ner)
                .await;
        }
    ...
    pub fn activate_hand(
        ...
        // Spawn the agent with a fixed ID for stable identity across restarts.
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        // cron scheduler. Idempotent via a marker key.
        {
            let kernel = Arc::clone(self);
            tokio::spawn(async move {
                kernel.migrate_shared_memory_schedules().await;
            });
        }
    ...
    pub(crate) async fn migrate_shared_memory_schedules(&self) {
        ...
        if let Ok(Some(serde_json::Value::Bool(true))) =
            self.memory.structured_get(shared, MARKER_KEY).await
        {
            return;
        }

        let entries: Vec<serde_json::Value> =
            match self.memory.structured_get(shared, LEGACY_KEY).await {
...
impl KernelHandle for OpenFangKernel {
    ...
    fn memory_list(
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(self.memory.list_kv(storage_id))
        })
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_migrate_shared_memory_schedules_imports_legacy_entries() {
        let tmp = tempfile::tempdir().unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_migrate_shared_memory_schedules_is_idempotent() {
        let tmp = tempfile::tempdir().unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_migrate_shared_memory_schedules_skips_unknown_agent() {
        let tmp = tempfile::tempdir().unwrap();
