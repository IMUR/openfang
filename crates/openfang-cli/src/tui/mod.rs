            ...
            "/approve" => {
                ...
                if id.is_empty() {
                    ...
                    let result = match &self.backend {
                        Backend::Daemon { base_url } => {
                            let full_id = resolve_approval_id_daemon(base_url, id);
                            match full_id {
                                Some(fid) => {
                                    let client = crate::daemon_client();
                                    client
                        ...
                        Backend::InProcess { kernel } => {
                            match resolve_approval_id_kernel(kernel, id) {
                                Some(uuid) => {
                    ...
                    self.chat.push_message(chat::Role::System, result);
                }
            }
            "/reject" => {
                let id = parts.get(1).map(|s| s.trim()).unwrap_or("");
...
/// Resolve a short approval-ID prefix to a full UUID via the daemon API.
fn resolve_approval_id_daemon(base_url: &str, prefix: &str) -> Option<String> {
    let client = crate::daemon_client();
    let resp = client
    ...
    body["approvals"].as_array().and_then(|arr| {
        arr.iter()
